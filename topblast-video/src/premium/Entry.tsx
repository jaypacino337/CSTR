...
import {FadeWords, HAIR, INK, SOFT, ZoneField} from './ui';

// 7–15.4s. Terminal → verified buy at $100 → entry line locks → $112 → $104 → $96.
...
const KP: [number, number][] = [
  [0, 88], [0.07, 90.5], [0.13, 89.2], [0.21, 94.6], [0.3, 100], [0.36, 105.5], [0.42, 104.2], [0.5, 112],
  [0.56, 108.4], [0.62, 104], [0.67, 105.1], [0.78, 96], [0.83, 97.2], [0.88, 95.6], [0.94, 96.8], [1, 96.2],
];
...
export const BUY_F = 50;
export const CROSS_F = 130;
export const PULL_F = 160;
export const headAt = (f: number) => keys(f, [20, BUY_F, 74, 100, 118, 140, 240], [0.001, 0.3, 0.3, 0.5, 0.62, 0.78, 0.94], inOut);
...
export const Entry: React.FC<{from: number}> = ({from}) => {
  ...
  const typedN = Math.max(0, Math.min(TYPED.length, Math.floor((f - 140) * 0.9)));
  ...
  const panelOut = prog(f, 232, 18, inOut);
  const zoneTitle = prog(f, 190, 34, expoOut);
  const titleOut = prog(f, 232, 16, inOut);
  ...
  return (
    <AbsoluteFill style={{background: INK}}>
      {g < 450 && <ZoneField g={g} reveal={prog(f, PULL_F + 6, 50, inOut)} lineY={lineY} />}
      ...
      >
        ...
        >
          ...
          <div style={{fontFamily: MONO, fontSize: 14, letterSpacing: '0.2em', color: SOFT, marginTop: 6}}>ENTRY</div>
          <div style={{fontFamily: MONO, fontWeight: 700, fontSize: 30, color: C.white}}>$100</div>
        </div>
      ...
      <div style={{position: 'absolute', left: 0, right: 0, top: 840, opacity: 1 - titleOut}}>
        <FadeWords
          segments={[
            ...
            {text: 'ENTER THE ZONE.', color: '#FFB27A'},
          ]}
          start={214}
          size={28}
          weight={600}
